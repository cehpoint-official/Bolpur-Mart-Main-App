...
export default function Orders() {
  ...
  useEffect(() => {
    const loadUserRatings = async () => {
      if (!user?.uid || !allProductIds.length) return;

      try {
        const ratingPromises = allProductIds.map(async (productId) => {
  ...
  const handleProductRating = async (
    ...
    if (userRatings[productId]) {
      toast.info(
        `You already rated "${productName}" with ${userRatings[productId]} stars`
      );
      return;
    }
    ...
    try {
      ...
      // Show success toast
      toast.success(
        `Thanks for rating "${productName}" with ${rating} star${
          rating !== 1 ? "s" : ""
        }!`,
        {
          duration: 3000,
