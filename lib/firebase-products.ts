...
export class FirebaseProductService {
    ...
    static getCurrentTimeSlotId(timeRules: TimeRulesConfig): string | null {
        ...
        for (const [slotId, slot] of Object.entries(timeRules)) {
            ...
            console.log(`Checking slot ${slot.timeSlotName}: ${slot.startTime} - ${slot.endTime}`)
    ...
    static getAllowedCategories(timeRules: TimeRulesConfig, currentSlotId: string): CategoryReference[] {
    ...
    static async getProducts(searchQuery?: string, categoryFilter?: string): Promise<Product[]> {
        try {
            ...
            querySnapshot.forEach((doc) => {
                ...
                if (!hasAllowedCategory) {
                    console.log(`Product ${product.name} doesn't have allowed categories`)
                    return
                }
                ...
                if (searchQuery && searchQuery.trim() !== '') {
                    ...
                    if (!matchesSearch) {
                        console.log(`Product ${product.name} doesn't match search: ${searchQuery}`)
                        return
                    }
                }
                ...
                if (categoryFilter && categoryFilter !== '') {
                    const hasSelectedCategory = product.categories.some(cat => cat.id === categoryFilter)
                    if (!hasSelectedCategory) {
                        console.log(`Product ${product.name} doesn't match category filter: ${categoryFilter}`)
                        return
                    }
                }
